...
while True:
    ...
    circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, dp=1.2, minDist=50,
                               ...
                               maxRadius=150)  # Set a maximum radius for large circles

    if circles is not None:
        circles = np.uint16(np.around(circles))  # Round the circle coordinates and radii
        for circle in circles[0, :]:
            # Get circle center and radius
            center_x, center_y, radius_px = circle[0], circle[1], circle[2]

            # Draw the circle in the image
            cv2.circle(undistorted_image, (center_x, center_y), radius_px, (0, 255, 0), 2)
            cv2.circle(undistorted_image, (center_x, center_y), 2, (0, 0, 255), 3)  # Mark the center
            ...
            cv2.putText(undistorted_image, f"Radius: {radius_mm:.2f}mm", 
                        (center_x - 50, center_y - radius_px - 10), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    else:
        cv2.putText(undistorted_image, "No circles detected", (50, 50), 
